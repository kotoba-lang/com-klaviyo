(ns klaviyo.main
  "Kotodama WASM entrypoint for the Klaviyo clean-room actor (L4) — Clojure port.
  ...
  facts (`klaviyo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "klaviyo")
(def tier "L4")
...
(def entity-specs
  [{:entity "Profile"   :plural "profiles"   :id-prefix "klaviyo_pro"
    :fields [:email :phoneNumber :externalId]  :required [:email :phoneNumber]
    :coerce {}                       :refs {}}
   {:entity "List"      :plural "lists"      :id-prefix "klaviyo_lis"
    :fields [:name]                            :required [:name]
    :coerce {}                       :refs {}}
   {:entity "Segment"   :plural "segments"   :id-prefix "klaviyo_seg"
    :fields [:name :isActive :isStarred]       :required [:name :isActive]
    :coerce {:isActive :bool :isStarred :bool} :refs {}}
   {:entity "Campaign"  :plural "campaigns"  :id-prefix "klaviyo_cam"
    :fields [:name :status :archived]          :required [:name :status]
    :coerce {:archived :bool}        :refs {}}
   {:entity "Flow"      :plural "flows"      :id-prefix "klaviyo_flo"
    :fields [:name :status :triggerType :archived] :required [:name :status]
    :coerce {:archived :bool}        :refs {}}
   {:entity "Event"     :plural "events"     :id-prefix "klaviyo_eve"
    :fields [:metricId :profileId :timestamp] :required [:timestamp]
    :coerce {:timestamp :int}        :refs {:profileId "Profile"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"klaviyo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "klaviyo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :klaviyo-compat/ready)
